...
> Source(GitHub) → Build(CodeBuild) → Deploy(ECS) 3단계로 구성된 파이프라인을 생성합니다.
        ...
        \"actions\": [
          {
            ...
            \"configuration\": {
              \"ClusterName\": \"ci-cd-demo-cluster\",
              \"ServiceName\": \"ci-cd-demo-service\",
...
**기대 결과**: Source → Build → Deploy 모두 **Succeeded**
...
**CodePipeline 콘솔**에서 Source → Build → Deploy 진행 확인
...
### Q: GitHub Actions 대신 CodePipeline을 사용하는 이유는?

**A**:
- 모든 CI/CD 리소스가 **AWS 내에서 완결**됩니다
- GitHub에 별도 Secrets 설정이 불필요합니다 (CodeStar Connections 사용)
- CodePipeline 콘솔에서 전체 파이프라인을 시각적으로 모니터링할 수 있습니다
- 기존에 CodePipeline을 사용하는 고객 환경과 동일한 경험을 제공합니다
...
**A**: CodePipeline의 Deploy 단계에서 ECS 서비스를 업데이트할 때 자동으로 실행됩니다.
...
- CodePipeline: V2 기준 사용한 만큼 과금 (프리 티어 있음)
- CodeBuild: 빌드 시간 기준 과금 (월 100분 프리 티어)
- Fargate: 태스크 수 x 실행 시간 x (vCPU + 메모리)
- CloudShell: **무료**

### Q: CloudFormation 템플릿의 GitHub Actions User는 뭔가요?

**A**: 원래 저장소가 GitHub Actions CI/CD도 지원하기 위해 포함된 리소스입니다. 이 워크샵에서는 CodePipeline을 사용하므로 **무시해도 됩니다**. 생성되더라도 추가 비용은 없습니다.
